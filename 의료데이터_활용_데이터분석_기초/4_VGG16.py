...
def VGG16():
    ...
    # 세 번째 Conv Block
    model.add(keras.layers.Conv2D(256, kernel_size = 3, activation= tf.nn.relu, padding= 'same'))
    model.add(keras.layers.Conv2D(256, kernel_size = 3, activation= tf.nn.relu, padding= 'same'))
    model.add(keras.layers.Conv2D(256, kernel_size = 3, activation= tf.nn.relu, padding= 'same'))
    model.add(keras.layers.MaxPooling2D(pool_size = 2, strides = 2))
    
    # 네 번째 Conv Block
    model.add(keras.layers.Conv2D(512, kernel_size = 3, activation= tf.nn.relu, padding= 'same'))
    model.add(keras.layers.Conv2D(512, kernel_size = 3, activation= tf.nn.relu, padding= 'same'))
    model.add(keras.layers.Conv2D(512, kernel_size = 3, activation= tf.nn.relu, padding= 'same'))
    model.add(keras.layers.MaxPooling2D(pool_size = 2, strides = 2))
